...
rec_gblinear = recipe( ~ ., data = tr_df) %>% 
  step_corr(all_numeric(), -massey_r_diff, threshold = .94) %>% 
  step_center(all_numeric()) %>% 
  step_scale(all_numeric()) %>% 
  prep()
...
dtest <- xgb.DMatrix(as.matrix(bake(rec2, te_df)),
                     label = te$target)
dtrain_gblinear <- xgb.DMatrix(as.matrix(bake(rec_gblinear, tr_df)),
                      label =  tr_lab)
dtest_gblinear <- xgb.DMatrix(as.matrix(bake(rec_gblinear, te_df)),
                     label = te$target)
# gbtree----
...
set.seed(1)
cv_l <- xgb.cv(params = paraml, dtrain_gblinear, nrounds = 10000, nfold = 20,
             early_stopping_rounds = 10,
             prediction = TRUE)

set.seed(1)
linear <- xgb.train(params = paraml, dtrain_gblinear, nrounds = 300)
...
submit <- submit %>% 
  mutate(pred_glm = predict(fit_glm, newdata = te_rec, type = "response"),
         ...
         pred_rf = predict(fit_rf, te_df)$predictions[,1],
         pred_ext = predict(fit_ext, te_df)$predictions[,1]) %>% 
  mutate(pred_rf = if_else(pred_rf < .025, .025, pred_rf),
         ...
         Pred_l = predict(linear, dtest_gblinear),
         Pred_b = predict(bst, dtest),
         Pred_d = predict(dart, dtest),) %>% 
  mutate(Pred = (Pred_l + pred_lasso  + pred_glm)/4+
                   (Pred_b + Pred_d + pred_rf + pred_ext + pred_ridge)/ 20
                 ) %>% 
  mutate(Pred2 = (Pred_l + pred_glm)/2)
